...
class Inline
{
        ...
        while ($i < $len) {
            ...
            switch ($sequence[$i]) {
                ...
                default:
                    ...
                    $value = self::parseScalar($sequence, $flags, array(
                        ',',
                        ']'
    ...
    private static function parseMapping($mapping, $flags, &$i = 0, $references = array())
    {
        ...
        while ($i < $len) {
            ...
            // key
            $isKeyQuoted = in_array($mapping[$i], array(
                '"',
                "'"
            ), true);
            $key = self::parseScalar($mapping, $flags, array(
                ':',
                ' '
            ), $i, false, array(), true);
            ...
            if (':' !== $key && ! $isKeyQuoted && (! isset($mapping[$i + 1]) || ! in_array($mapping[$i + 1], array(
                ' ',
                ',',
                '[',
                ']',
            ...
            while ($i < $len) {
                ...
                switch ($mapping[$i]) {
                    ...
                    case '{':
                        ...
                        break;
                    default:
                        $value = self::parseScalar($mapping, $flags, array(
                            ',',
                            '}'
                        ), $i, null === $tag, $references);
                        // Spec: Keys MUST be unique; first one wins.
            ...
            case '+' === $scalar[0] || '-' === $scalar[0] || '.' === $scalar[0] || is_numeric($scalar[0]):
                switch (true) {
                    ...
                    case Parser::preg_match('/^(-|\+)?[0-9][0-9_]*(\.[0-9_]+)?$/', $scalar):
                        ...
                        return (float) str_replace(array(
                            ',',
                            '_'
                        ), '', $scalar);
